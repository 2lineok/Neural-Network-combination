...
for epoch in range(num_epochs):
    ...
    loss = lamda*criterion(outputs1, targets1) +lamda*( criterion(aa[:,1:-1] ,bb[:,1:-1])+1000*(criterion( outputs[:,0],-x[:,0])+criterion( outputs[:,-1],-x[:,-1]+1)))  # targets are your target values
    ...
    if epoch % 10000 == 0:
        ...
        torch.save(model, os.path.join(directory,'deep_onet_modelch12.pth'))

        # Save the model's state dictionary (recommended)
        torch.save(model.state_dict(), os.path.join(directory,  'deep_onet_model_state_dictch12.pth'))

        # Save the optimizer's state (optional)
        torch.save(optimizer.state_dict(), os.path.join(directory, 'deep_onet_optimizer_state_dictch12.pth'))
